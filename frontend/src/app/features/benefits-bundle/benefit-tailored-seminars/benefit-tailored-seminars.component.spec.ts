...
import { TailoredSeminarsComponent } from './benefit-tailored-seminars.component';

describe('TailoredSeminarsComponent', () => {
  const pages = MockPagesService.pagesFactory();

  async function setup() {
    const { fixture, getByText, queryByText } = await render(TailoredSeminarsComponent, {
  ...
  it('should display the title', async () => {
    ...
    const title = getByText(`10% off tailored seminars from Skills for Care`);

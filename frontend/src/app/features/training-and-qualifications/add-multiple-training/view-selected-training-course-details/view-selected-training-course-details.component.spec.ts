...
fdescribe('ViewSelectedTrainingCourseDetailsComponent', () => {
  const workplace = {
    uid: '1',
  }
  ...
  // let courseCompletionDateDay = null;
  // let courseCompletionDateMonth = null;
  // let courseCompletionDateYear = null;
  ...
  describe('Continue button', () => {
    ...
    describe('When there are no validation errors', () => {
      describe('When a course completion date is entered', () => {
        it(`should call the training service with the course completion date`, async () => {
          ...
          dayInputBox.value = courseCompletionDateDay;
          dayInputBox.dispatchEvent(new Event('input'));

          monthInputBox.value = courseCompletionDateMonth;
          monthInputBox.dispatchEvent(new Event('input'));

          yearInputBox.value = courseCompletionDateYear;
      ...
      describe('When a course completion date is not entered', () => {
        it('should not call the training service', async () => {
          ...
          expect(setCourseCompletionDateSpy).not.toHaveBeenCalledWith(courseCompletionDate);
        });
      })
      ...
      describe('When notes are not entered', () => {
        it(`should not call the training service with the notes`, async () => {
          const { getByRole, setNotesSpy } = await setup();

          const button = getByRole('button', { name: 'Continue' });
          button.click();

          expect(setNotesSpy).not.toHaveBeenCalledWith(notes);
        });
      })
    ...
    describe('When there are validation errors', () => {
      it('should not navigate away from the page', async () => {
        ...
        const dateInputBox = fixture.nativeElement.querySelector('input');
        dateInputBox.value = '01';
      ...
      describe('when a future date is entered in the completion date', () => {
        it('should raise the correct error message ', async () => {
          ...
          monthInputBox.value = '12';
          monthInputBox.dispatchEvent(new Event('input'));

          yearInputBox.value = '2125';
          yearInputBox.dispatchEvent(new Event('input'));
...
// tidy code
//  add tests for date picker component

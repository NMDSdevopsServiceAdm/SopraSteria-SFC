...
exports.WorkerMainJobProperty = class WorkerMainJobProperty extends ChangePropertyPrototype {
  ...
  restorePropertyFromSequelize(document) {
    if (document.mainJob) {
      return {
        ...
        canDoDelegatedHealthcareActivities: document.mainJob.canDoDelegatedHealthcareActivities,
      };
    }
  }
  ...
  isEqual(currentValue, newValue) {
    ...
    return (
      currentValue &&
      newValue &&
      currentValue.jobId === newValue.jobId &&
  ...
  async _validateJob(jobDef) {
    ...
    if (jobDef.jobId) {
      referenceJob = await models.job.findOne({
        ...
        attributes: ['id', 'title', 'other', 'canDoDelegatedHealthcareActivities'],
      });
    } else {
      referenceJob = await models.job.findOne({
        where: {
    ...
    if (referenceJob && referenceJob.id) {
      ...
      if (referenceJob.other && jobDef.other && jobDef.other.length && jobDef.other.length > OTHER_MAX_LENGTH) {
        ...
        return {
          ...
          canDoDelegatedHealthcareActivities: referenceJob.canDoDelegatedHealthcareActivities,
        };
      }
    } else {
      return false;

...
class Training extends EntityValidator {
  constructor(establishmentId, workerUid) {
    ...
    this._trainingProvider = null;
    this._trainingProviderFk = null;
    this._externalProviderName = null;
    this._otherTrainingProviderName = null;
  ...
  get otherTrainingProviderName() {
    return this._otherTrainingProviderName;
  }
  ...
  set trainingProvider(trainingProvider) {
    this._trainingProvider = trainingProvider;
  }
  ...
  set otherTrainingProviderName(otherTrainingProviderName) {
    this._otherTrainingProviderName = otherTrainingProviderName;
  }
  ...
  async validateTrainingRecord(document) {
    ...
    if (!trainingCategories || !Array.isArray(trainingCategories)) {
      this._validations.push(
        new ValidationMessage(ValidationMessage.ERROR, 100, 'Failed to get all training categories', [
          'TrainingCategory',
        ]),
      );
    ...
    if (document.trainingCategory) {
      ...
      if (!(document.trainingCategory.id || document.trainingCategory.category)) {
        ...
        this._log(
          Training.LOG_ERROR,
          'category failed validation: trainingCategory.id or trainingCategory.category must exist',
        );
        returnStatus = false;
      }
      ...
      if (document.trainingCategory.id && !Number.isInteger(document.trainingCategory.id)) {
        ...
        this._log(
          Training.LOG_ERROR,
          `category failed validation: trainingCategory.id (${document.trainingCategory.id}) must be an integer`,
        );
        returnStatus = false;
      }
      ...
      if (foundCategory === null || foundCategory === undefined) {
        ...
        this._log(
          Training.LOG_ERROR,
          `category failed validation: trainingCategory.id (${document.trainingCategory.id}) or trainingCategory.category (${document.trainingCategory.category}) must exist`,
        );
        returnStatus = false;
      } else {
    ...
    if (document.title) {
      ...
      if (document.title.length > MAX_LENGTH) {
        this._validations.push(
          new ValidationMessage(ValidationMessage.ERROR, 110, `validation: MAX(${MAX_LENGTH}) length`, ['Title']),
        );
    ...
    if (document.accredited) {
      ...
      if (!ALLOWED_VALUES.includes(document.accredited)) {
        this._validations.push(
          new ValidationMessage(ValidationMessage.WARNING, 120, `unexpected value - ${document.accredited}`, [
            'Accredited',
          ]),
        );
        this._log(
          Training.LOG_ERROR,
          `accredited failed validation: accredited failed validation: unexpected value - ${document.accredited}`,
        );
        returnStatus = false;
      }
    ...
    if (document.otherTrainingProviderName) {
      validatedTrainingRecord.otherTrainingProviderName = document.otherTrainingProviderName;
    } else {
      validatedTrainingRecord.otherTrainingProviderName = null;
    }
    ...
    if (document.completed) {
      ...
      if (!expectedDate.isValid()) {
        this._validations.push(
          new ValidationMessage(ValidationMessage.ERROR, 130, `unexpected date - ${document.completed}`, ['Completed']),
        );
      ...
      if (!expectedDate.isSameOrBefore(moment(), 'day')) {
        this._validations.push(
          new ValidationMessage(ValidationMessage.ERROR, 131, `must be in the past - ${document.completed}`, [
            'Completed',
          ]),
        );
    ...
    if (document.expires) {
      ...
      if (!expectedDate.isValid()) {
        this._validations.push(
          new ValidationMessage(ValidationMessage.ERROR, 140, `unexpected date - ${document.expires}`, ['Expires']),
        );
      ...
      if (validatedTrainingRecord.completed && !expectedDate.isAfter(validatedTrainingRecord.completed, 'day')) {
        this._validations.push(
          new ValidationMessage(ValidationMessage.ERROR, 141, `must be in the past - ${document.expires}`, ['Expires']),
        );
    ...
    if (document.notes) {
      ...
      if (document.notes.length > MAX_LENGTH) {
        this._validations.push(
          new ValidationMessage(ValidationMessage.WARNING, 150, `validation: MAX (${MAX_LENGTH}) length`, ['Notes']),
        );
  ...
  async load(document) {
    try {
      ...
      if (validatedTrainingRecord !== false) {
        ...
        this.otherTrainingProviderName = validatedTrainingRecord.otherTrainingProviderName;
      ...
      try {
        ...
        if (workerRecord && workerRecord.id) {
          ...
          const creationDocument = {
            ...
            otherTrainingProviderName: this._otherTrainingProviderName,
  ...
  async restore(uid) {
    ...
    try {
      ...
      if (fetchResults && fetchResults.id && Number.isInteger(fetchResults.id)) {
        ...
        this._trainingProvider = fetchResults.trainingProvider;
        this._trainingProviderFk = fetchResults.trainingProviderFk;
        this._externalProviderName = fetchResults.otherTrainingProviderName
          ? fetchResults.otherTrainingProviderName
          : fetchResults.trainingProvider?.name;
  ...
  async delete() {
    if (this._workerUid === null || this._establishmentId === null) {
      this._log(
        Training.LOG_ERROR,
        'Cannot delete a training record having unknown establishment uid, worker uid or training uid',
      );
  ...
  toJSON() {
    ...
    const myDefaultJSON = {
      ...
      trainingProviderFk: this.trainingProviderFk ? this.trainingProviderFk : null,
  ...
  static async getAllRequiredCounts(establishmentId, workerRecords, categoryId = null) {
    if (workerRecords.length !== 0) {
      ...
      for (let i = 0; i < workerRecords.length; i++) {
        ...
        if (categoryId === null) {
          worker.missingMandatoryTrainingCount = await Training.getAllMissingMandatoryTrainingCounts(
            establishmentId,
            worker,

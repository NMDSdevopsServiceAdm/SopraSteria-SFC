...
const addTypeContent = async (notification) => {
    ...
    switch (notification.type) {
      ...
      case 'BECOMEAPARENT': {
        ...
        console.log('BECOMEAPARENT');
        if (becomeAParentNotificationDetails) {
          notification.typeContent = {
            approvalStatus: becomeAParentNotificationDetails.Status.toUpperCase(),
          };
        }
        break;
      }
  ...
  const getNotification = async (req, res) => {
    try {
      const notificationData = getOneNotification(req.params.notificationUid).notification;

      await addTypeContent(notificationData);

      // this will fetch notification receiver name
      if (notificationData.type === 'OWNERCHANGE') {
        const notificationReciever = await ownershipChangeRequests.getNotificationRecieverName(notificationData);

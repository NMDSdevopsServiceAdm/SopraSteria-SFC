...
router.route('/').get(async (req, res) => {
  ...
  try {
    ...
    if (results && Array.isArray(results) && results.length > 0) {
      res.status(200);
      return res.json(LaFormatters.listOfLAsJSON(results.map(thisCssr => { return {cssrId: thisCssr.id, name: thisCssr.name};})));
    } else {
...
router.route('/:postcode').get(async (req, res) => {
  ...
  try {
    ...
    if (cssrResults && cssrResults.postcode === givenPostcode &&

...
router.route('/signedUrl').get(async function (req, res) {
  ...
  try {
    var uploadPreSignedUrl = s3.getSignedUrl('putObject', {
      ...
      Key: establishmentId + '/new/' + req.query.filename,
      // ACL: 'public-read',
      ...
      Metadata: {
        ...
        filestatus: FileStatusEnum.New,
        validationstatus: FileValidationStatusEnum.Pending
      },
      Expires: 300, // 5 minutes
    });
    res.json({ success: true, message: 'Pre-Signed', urls: uploadPreSignedUrl });
    res.end();
  }
  catch (err) {
    console.error('establishment::bulkupload GET/:PreSigned - failed', err.message);
    return res.status(503).send('Failed to restrieved Presigned URL');
  }
});

...
const _mapResults = async (approvalResults) => {
    const promises = approvalResults.map( async approval => {
        data = approval.Data;
        const currentServiceID = data.currentService || null;
        const requestedServiceID = data.requestedService || null;
...
const _approveChange = async (req, res) => {
  ...
  await _updateApprovalStatus(req.body.cqcStatusChangeRequestId, 'Approved');
  await _updateMainService(req.body.cqcStatusChangeRequestId);
...
const _rejectChange = async (req, res) => {
  // await _notify(req.body.cqcStatusChangeId, req.userUid, req.body.establishmentId);
  await _updateApprovalStatus(req.body.cqcStatusChangeId, 'Rejected');
...
const _updateMainService = async (approvalId) => {
  let singleApproval = await models.Approvals.findbyId(approvalId);
  let workplace = await models.establishment.findbyId(singleApproval.EstablishmentID);
  let requestedServiceFK = singleApproval.data.requestedService;
  if (workplace) {
    workplace.MainServiceFKValue = requestedServiceFK;

...
describe('Bulk upload', () => {
  ...
  before(async () => {
    ...
    const registration = await apiEndpoint
      ...
      .expect(200);
    if (registration) {
      const adminLogin = await apiEndpoint.post('/login').send(admin).expect('Content-Type', /json/).expect(200);
  ...
  describe('[PUT] bulkupload.amazonaws.com/:presigned', () => {
    it('should upload org csv to S3 bucket', async () => {
      await supertest('').put(awsLinkOrg).send(orgCsv).expect(200);
    });
    it('should upload staff csv to S3 bucket', async () => {
      await supertest('').put(awsLinkStaff).send(orgCsv).expect(200);
    });
  });
  ...
  describe('/api/establishment/:establishmentuid/bulkupload/uploaded/:establishmentid/latest/:filename', () => {
    it('should return an an object file information for establishment including errors, warnings etc.', async () => {
      const download = await apiEndpoint
        .get(
          `/establishment/${encodeURIComponent(establishmentUid)}/bulkupload/uploaded/${encodeURIComponent(
            establishmentId,
          )}/latest/org-test.csv`,
        )
        .set('Authorization', loginAuth)
  ...
  describe('/api/establishment/:establishmentuid/bulkupload/report/establishments', () => {
    ...
    it('should fail if trying to validate files with no authorization token passed ', async () => {
      await apiEndpoint
        .put(`/establishment/${encodeURIComponent(establishmentUid)}/bulkupload/report/establishments`)
        .expect(401);
    });
    it('should fail if trying to validate somone elses ', async () => {
      await apiEndpoint
  ...
  describe('/api/establishment/:establishmentuid/bulkupload/report/workers', () => {
    ...
    it('should fail if trying to validate files with no authorization token passed ', async () => {
      await apiEndpoint
        .put(`/establishment/${encodeURIComponent(establishmentUid)}/bulkupload/report/workers`)
        .expect(401);
    });
    it('should fail if trying to validate somone elses ', async () => {
      await apiEndpoint
  ...
  describe('/api/establishment/:establishmentuid/bulkupload/report/training', () => {
    ...
    it('should fail if trying to validate files with no authorization token passed ', async () => {
      await apiEndpoint
        .put(`/establishment/${encodeURIComponent(establishmentUid)}/bulkupload/report/training`)
        .expect(401);
    });
    it('should fail if trying to validate somone elses ', async () => {
      await apiEndpoint

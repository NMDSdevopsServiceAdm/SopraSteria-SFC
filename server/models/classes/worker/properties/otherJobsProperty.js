...
exports.WorkerOtherJobsProperty = class WorkerOtherJobsProperty extends ChangePropertyPrototype {
  ...
  async restoreFromJson(document) {
    if (document.otherJobs) {
      if (
        Array.isArray(document.otherJobs.jobs) &&
        document.otherJobs.jobs.length > 0 &&
        document.otherJobs.value === 'Yes'
      ) {
    ...
    if (
      ...
      if (currentValue.jobs.length == newValue.jobs.length) {
        ...
        arraysEqual = currentValue.jobs.every((thisJob) => {
          return newValue.jobs.find(
            (newJob) =>
              newJob.jobId === thisJob.jobId && (newJob.other === thisJob.other || (!newJob.other && !thisJob.other)),
          );
        });
      } else {

...
export class NumberMax extends BaseNumber {
  ...
  onKeyDown(event: KeyboardEvent) {
    ...
    if ((isFinite(parseInt(key)) && !this.isSelectionTypeRange()) || this.isKeyCodeIncrement(keyCode)) {
      ...
      if ((this.isKeyCodeIncrement(keyCode) && curValNum + 1 > this.max) || nextValNum > this.max) {

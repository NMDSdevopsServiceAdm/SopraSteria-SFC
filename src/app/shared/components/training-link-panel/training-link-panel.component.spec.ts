...
describe('TrainingLinkPanelComponent', () => {
  ...
  it('should show the `change when you get expires soon alert` link when canEditworker in permissions service is true', async () => {
    const { component, fixture, componentInstance } = await setup();

    componentInstance.canEditWorker = true;
    fixture.detectChanges();

    expect(component.getByText('Change when you get `expires soon` alerts')).toBeTruthy();
  });

  it('should not show the `change when you get expires soon alert` link when canEditworker in permissions service is false', async () => {
    const { component, fixture, componentInstance } = await setup();

    componentInstance.canEditWorker = false;
    fixture.detectChanges();
